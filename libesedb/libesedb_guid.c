...
{
	static char *function = "libesedb_guid_to_string";
	int print_count       = 0;
	...
	if( ( byte_order != LIBESEDB_ENDIAN_BIG )
	 && ( byte_order != LIBESEDB_ENDIAN_LITTLE ) )
	{
		liberror_error_set(
		 error,
		 LIBERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	if( string_size < LIBESEDB_GUID_STRING_SIZE )
	{
		liberror_error_set(
		 error,
		 LIBERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	else if( byte_order == LIBESEDB_ENDIAN_LITTLE )
	{
		print_count = narrow_string_snprintf(
			       (char *) string,
			       string_size,
