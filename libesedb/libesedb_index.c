     ...
     liberror_error_t **error )
{
	...
	if( *index != NULL )
	{
		...
		*index         = NULL;
		...
		/* The internal_file and index_definition references
		 * are freed elsewhere
		 */
     ...
     liberror_error_t **error )
{
	...
	if( internal_index != NULL )
	{
		if( ( (libesedb_internal_index_t *) internal_index )->internal_file != NULL )
		{
			memory_free(
...
/* Attaches the index to the file
 * Returns 1 if successful or -1 on error
 */
     ...
     liberror_error_t **error )
{
	     ...
	     error ) != 1 )
	{
		liberror_error_set(
		 ...
		 "%s: unable to append internal index to file.",
		 function );

		return( -1 );
	}
	internal_index->internal_file    = internal_file;
	internal_index->index_definition = index_definition;

	return( 1 );
}

/* Detaches the index from its file reference
 * Returns 1 if successful or -1 on error
 */
     ...
     liberror_error_t **error )
{
	...
	if( internal_index->internal_file != NULL )
	{
		...
		if( libesedb_list_remove_element(
		     internal_index->internal_file->index_reference_list,
		     internal_index->list_element,
		     error ) != 1 )
		{
			liberror_error_set(
			 ...
			 LIBERROR_ERROR_DOMAIN_RUNTIME,
			 LIBERROR_RUNTIME_ERROR_REMOVE_FAILED,
			 "%s: unable to remove internal index from file.",
			 function );

			return( -1 );
		}
		internal_index->internal_file    = NULL;
		internal_index->index_definition = NULL;
	}
	return( 1 );
}
     ...
     liberror_error_t **error )
{
	...
	if( internal_index->index_definition == NULL )
	{
		liberror_error_set(
		 error,
		 LIBERROR_ERROR_DOMAIN_RUNTIME,
		 LIBERROR_RUNTIME_ERROR_VALUE_MISSING,
		 "%s: invalid internal index - missing index definition.",
		 function );
	...
	*identifier = internal_index->index_definition->father_data_page_object_identifier;
     ...
     liberror_error_t **error )
{
	...
	*utf8_string_size = internal_index->index_definition->name_size;
     ...
     liberror_error_t **error )
{
	...
	if( utf8_string_size < internal_index->index_definition->name_size )
	{
		liberror_error_set(
		 error,
		 LIBERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	if( memory_copy(
	     utf8_string,
	     internal_index->index_definition->name,
	     internal_index->index_definition->name_size ) == NULL )
	{
		liberror_error_set(
		 error,
		 LIBERROR_ERROR_DOMAIN_MEMORY,
