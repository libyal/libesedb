     ...
     liberror_error_t **error )
{
	...
	if( libesedb_array_initialize(
	     ...
	     column_data_definition_list->amount_of_elements,
	     error ) != 1 )
	{
		liberror_error_set(
		 error,
	     ...
	     list_element_iterator++ )
	{
		...
		data_type_definition->identifier = column_data_definition->identifier;
		data_type_definition->type       = column_data_definition->column_type;
		...
		else
		{
			...
			if( ( remaining_definition_data_size > 0 )
			 && ( column_data_definition->identifier == tagged_data_type_identifier ) )
			{
#if defined( HAVE_DEBUG_OUTPUT )
				libnotify_verbose_printf(
