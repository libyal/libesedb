/*
 * Library page_tags_value type test program
 *
...
#include "../libesedb/libesedb_page_tags_value.h"

#if defined( __GNUC__ ) && !defined( LIBESEDB_DLL_IMPORT )

/* Tests the libesedb_page_tags_value_initialize function
 * Returns 1 if successful or 0 if not
 */
int esedb_test_page_tags_value_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libesedb_page_tags_value_initialize(
	          &page_tags_value,
	          &error );
	...
	result = libesedb_page_tags_value_free(
	          &page_tags_value,
	          &error );
	...
	page_tags_value = (libesedb_page_tags_value_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libesedb_page_tags_value_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libesedb_page_tags_value_initialize with memset failing
		 */
...
/* Tests the libesedb_page_tags_value_free function
 * Returns 1 if successful or 0 if not
 */
int esedb_test_page_tags_value_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	ESEDB_TEST_RUN(
	 "libesedb_page_tags_value_initialize",
	 esedb_test_page_tags_value_initialize );

	ESEDB_TEST_RUN(
	 "libesedb_page_tags_value_free",
	 esedb_test_page_tags_value_free );
